...
func (widget *Widget) loadVmf(filename string) {
	...
	widget.model.Project.Vmf = sceneModel
	widget.model.Project.Filename = filename

	for i := 0; i < widget.model.Project.Vmf.Entities().Length(); i++ {
		widget.dispatcher.Dispatch(events.NewEntityCreated(widget.model.Project.Vmf.Entities().Get(i)))
	}

	for i := 0; i < len(widget.model.Project.Vmf.Worldspawn().Solids); i++ {
		widget.dispatcher.Dispatch(events.NewNewSolidCreated(&widget.model.Project.Vmf.Worldspawn().Solids[i]))
	}

	for i := 0; i < len(widget.model.Project.Vmf.Cameras().CameraList); i++ {
		widget.dispatcher.Dispatch(events.NewNewCameraCreated(&widget.model.Project.Vmf.Cameras().CameraList[i]))
	}
	if widget.model.Project.Vmf.Cameras().ActiveCamera != -1 {
		widget.dispatcher.Dispatch(events.NewCameraChanged(&widget.model.Project.Vmf.Cameras().CameraList[widget.model.Project.Vmf.Cameras().ActiveCamera]))
	}
